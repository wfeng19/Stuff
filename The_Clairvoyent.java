...
public class The_Clairvoyent {
	...
	static String[] weather_r = new String[]
			...
			"Can you please retype your answer?"
			};
	...
	public static void main(String[] args) {
		...
		printArray(dialogue);
		while (!isCorrectAnswer(input.nextLine()));
        ...
        while (!validInput){
        	...
        	yes = (answer).equalsIgnoreCase(choices[0]);
        	if (yes){
        		...
        		System.out.println("I have no idea what you mean, but I will continue, I guess.");
        	}
        }
        validInput = false;
	...
	static boolean isCorrectAnswer(String userAnswer) {
	    boolean rightAnswer = (userAnswer).equalsIgnoreCase(choices[0]);
	    if (rightAnswer){
	    	System.out.println("Thank you, and let the questions begin!");
	    }
	    else if (!rightAnswer){
	    	System.out.println("Allow me to repeat myself then.");
	    	printArray(dialogue);
	    }
	    return rightAnswer;
	}
